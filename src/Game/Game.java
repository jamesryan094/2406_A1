...
/**
 * Instantiates Game object that represents the state of any Mineral Supertrumps Game.
 * Attributes are manipulated through method calls from an adjacent "controller" file; RunGame.
 * Class also stores large string to be printed in the Game's menu and String constants.
 *
...
class Game {
    ...
    /**
     * Game constructor takes the desired number of players for the game, as well as
     * a User's username, and creates the appropriate number of human and nonHuman players.
     * The userName is assigned to the human player's name field.
     * Relevant values are initialised to ensure the first turn has its appropriate conditions followed.
     *
     *
    ...
    /**
     * Accesses the numPlayers field to determine the number of player objects to make,
     * returns a Player array with this many players, one of them being a human player,
     * with the passed username as their name.
    ...
    /**
     * Generates random number within range (0 - numPlayers, (inclusive)),
     * assigns the player at this index in Players array as the dealer for the game.
     */
    void assignDealer(){
    ...
    /**
     * The rule cards are first removed from the deck and the deck is shuffled.
     * The NUM_CARDS_PER_HAND constant is accessed to determine how many cards to deal,
     * the Player array field is accessed to determine who to deal to.
     * Dealing involves removing the a Card from the game's deck attribute
...
//        Dealer set as current player so that "player to the left of the dealer" goes first, as per the rules.
    ...
    /**
     * Sets the current player to the next person in the Player array.
     * If the previous player was the last in the array, the current player is reset to the first.
    ...
    /**
     * Handles the decision process for a nonHuman player playing the first turn of a round or the game.
     * will attempt to play combo, if it can not then any playable card.
     * The attributes of the card in play are updated appropriately.
     */
    void playFirstTurn(){
//        When a robot plays first card of round:
    ...
    /**
     * Called only when a user is the first player of a round and is playing a mineral card or "The Geologist".
     * Prompts the user for a trump category choice then updated the value and category in play.
...
//        When a user is playing a MineralCard Or 'The Geologist':
    ...
    /**
     * Called only when a user plays a trump card that is not "The Geologist".
     * Method will set the current trump category based on card title.
...
//        When a user plays a Trump Card:
    ...
    /**
     * Handles the decision process for a nonHuman player playing any turn that
     * is not the first turn of a round or the game.
     * will attempt to play combo, if it can not then any playable card.
     * The attributes of the card in play are updated appropriately.
     * If the player has no playable cards, they pass.
     */
    void playTurn() {
     ...
     * Will set value to end game if all but one player in list
     */
    void updateWinners() {
    ...
    /**
     * Checks how many of the players in the game have passed.
     * If this number is one less than the number of players who have not already won, the round is reset.
