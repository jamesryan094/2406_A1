...
public class NonHumanPlayer extends Player {
     ...
     * As per the game's rules, any card can be played while beginning a round (bar the very first turn).
     * As such, a random card is picked.
    ...
    /**
     * Goes through player's hand and checks each card's type.
     * Returns the first card of mineral type.
    ...
    /**
     * Compares the last played card in the game to each card in the nonHuman player's hand to ultimately determine
     * weather the current player must pass or not.
    ...
    /**
     * Generates a random number between 0 and the number of trump categories (5).
     * return a trump choice as a string, corresponding the the random integer.
