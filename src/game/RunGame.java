...
/**
 * Acts as conduit between player and system by taking input and print output, controls flow of
 * game. Instantiates Game object then manipulates attributes based on actions of human and nonHuman
 * players. Establishes rules of the game Error checks input from user and provides output for user.
 *
...
public class RunGame {
  ...
  private static final String TURN_MENU_MESSAGE =
      "(0)Inspect Hand\n(1)Play Card\n(2)Combo\n(3)Pass";
                ...
                if (!newGame.isHumanUp()) {
                  ...
                  if (isNewRound || newGame.isFirstTurn()) {
                    ...
                    if (currentPlayer.hasPlayableCards(
                        newGame.getLastPlayedCard(), newGame.getCurrentTrumpCategory())) {
                      newGame.playTurn();
                      displayTurnResults(newGame);
                    } else {
                  ...
                  if (newGame.checkWinner()) {
                    ...
                    System.out.println(
                        "Congratulations "
                            ...
                            + "! You have been added to the winner list!");
                  }
                  if (!comboPlayed) {
                    ...
                    System.out.println(
                        currentPlayer.getName()
                            + " played the COMBO! They win the round and get to go again.");
                    newGame.playFirstTurn();
                  ...
                  while (newGame.isHumanUp()) {
                    ...
                    switch (turnChoice) {
                      case 0:
                        ...
                        displayHandMenu(
                            currentPlayer.getHand(), turnChoice, newGame, isNewRound, comboPlayed);
                        break;
                      case 1:
  ...
  /**
   * Prints the currently selected trump category and the corresponding trump value of the most
  ...
  private static void displayTurnResults(Game newGame) {
    Player currentPlayer = newGame.getCurrentPlayer();
    System.out.println(
        currentPlayer.getName() + " Played: " + newGame.getLastPlayedCard().getTitle());
   ...
   * playing by setting setting a dealer, dealing cards to each player, and setting a current, first
   * player.
   *
  ...
  /**
   * Prompts user for username. Username is valid if input is not blank or not exclusively
   * whitespace.
   *
  ...
  /**
   * Prompts user for Menu Choice. Menu Choice is valid if input is one of 4 predetermined
   * characters. Will constantly reprompt the user until a valid character is entered (not case
   * sensitive).
   *
  ...
  /**
   * Checks input when user is to select a whole integer value in a range. Returns an integer
   * representing a menu choice.
   *
  ...
  /**
   * Provides main functionality for user when playing their turn. A list of options are displayed
   * to user. From here user can choose to inspect card details, play a card, play the combo or
   * pass.
   *
   * @param userHand the human players current hand of cards as an ArrayList cast to Card objects.
   * @param menuChoice the intention of the user. HandMenu will decide how to respond to a card
   *     choice based on the previous menu choice (inspect or play) @see turnMenu
   * @param newGame the game object.
   * @param isNewRound determines weather human player is to pick a trump category after playing a
   *     mineral card or not
   * @param comboPlayed allows a human to play another card after playing a combo.
   */
  private static void displayHandMenu(
  ...
  /**
   * Prompts user for trump category choice as int. Validates input with getNumInRange() and returns
   * a valid trump choice.
   *
   * @return a string representing the trump value selected to play.
   */
  private static String getTrumpCategoryFromUser() {
    int trumpChoice;
    String TRUMP_CATEGORY_MESSAGE =
